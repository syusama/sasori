...
| Tool progress | Tool execution update callback with settle-time rejection of late updates | Bounded, transient `ToolProgressEvent` values through an explicit keyword-only `ToolExecutionContext`; progress never enters Store, provider input, approval, fingerprints, or recovery | Ordered live feedback; immutable payloads; return/raise/timeout/cancel fence late worker updates |
