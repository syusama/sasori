...
> 研究日期：2026-08-09
>
> Sasori 对标前已托管验证基线：[`6658332`](https://github.com/syusama/sasori/commit/6658332ade12aa2b558aa0db5de6218c2eafd8c8)
>
> Sasori 本次候选：本文所在 revision；新增 Context/README 证据先标记为本地候选，不能追溯成 `6658332` 已交付能力
>
...
Sasori 的既有 Hosted 结论只绑定 `6658332`，本次新增能力绑定本文所在候选源码与
...
不得在当前 README 中宣称：已具备完整语义 Memory、通用 Artifact 链、
Workflow、多 Agent、插件沙箱、水平扩展、中央市场或正式签名发布。
...
| Context | 本次候选已实现核心外、结构安全的确定性预算投影与拒绝调用恢复；本地回归已通过，尚不追溯到 `6658332` Hosted | ContextSource、预算、压缩与 recall 比 Sasori 全面 | 三层 compaction、cache-stable prefix、token 压力与 archive 很强 | ToFu 胜；Sasori 候选有可靠底座 |
...
| Artifact/FileRef | 尚无通用契约/下载/预览链 | `FileRef`、FileService、预签名预览/下载、工作流资产复用 | Artifacts/Canvas、产物仓与 content references 较丰富 | Sasori 落后 |
...
| 测试证据 | `6658332` 已有 Windows/Linux × Python 3.11–3.13、源码/wheel/sdist/容器/浏览器 Hosted 门禁；本次候选需重新取得 exact-revision Hosted 结果 | 测试覆盖广，但 README claim 与 release/container 证据未系统绑定 | 测试规模巨大，但当前固定 HEAD 的 hosted workflow 非绿 | Sasori 基线发布证据胜；候选待刷新 |
...
| 结构安全 context budget | 本次 | under/over budget、parallel calls、orphan/incomplete、custom estimator、adapter 回归 |
| ArtifactRef + immutable blob metadata + run association | 下一项 | filename/MIME 不可信、digest/size 校验、原子写、range/download、跨重启、未授权 run 拒绝 |
...
| Artifact Workbench | 图片/文本/PDF 安全预览与下载；CSP；冷加载；浏览器真实用户链 |
