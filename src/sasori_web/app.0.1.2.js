...
const state = {
  ...
  run: null,
  eventState: null,
  runGate: createRunGate(),
};

function activateRun(runId) {
  const context = state.runGate.activate(runId);
  state.eventState = createEventState(runId);
  return context;
}

function currentRunContext() {
  return state.runGate.current();
}

function contextIsActive(context) {
  return Boolean(context && state.runGate.isActive(context));
}

function watcherIsActive(watcher) {
  return Boolean(watcher && state.runGate.isWatcherActive(watcher));
}

function startWatching(context) {
  if (!contextIsActive(context)) return;
  const watcher = state.runGate.startWatcher(context);
  watchRun(watcher).catch((error) => {
    if (watcherIsActive(watcher)) showError(error, "事件流失败");
  }).finally(() => state.runGate.stopWatcher(watcher));
}

function eventCursor() {
  return state.eventState ? state.eventState.cursor : 0;
}
...
function showError(error, context) {
  ...
  const code = error && typeof error.code === "string" ? error.code : "client_error";
...
function setRunProjection(projection, context) {
  if (!contextIsActive(context)) return false;
  if (!projection || projection.run_id !== context.runId) {
    throw new Error("run projection does not match the active view");
  }
  if (state.run && state.run.run_id === projection.run_id &&
      Number.isInteger(state.run.revision) && Number.isInteger(projection.revision) &&
      projection.revision < state.run.revision) return false;
  if (!state.eventState || state.eventState.runId !== projection.run_id) {
    state.eventState = createEventState(projection.run_id);
  }
  state.run = projection;
  $("#active-run-label").textContent = projection ? projection.run_id : "—";
  $("#sequence-label").textContent = String(eventCursor());
...
function addEvent(projected) {
  if (!state.eventState) return false;
  const next = reduceEvent(state.eventState, projected);
  if (next === state.eventState) return false;
...
function renderTimeline() {
  ...
  list.replaceChildren();
  const events = state.eventState ? state.eventState.events : [];
...
async function resolveApproval(approved) {
  ...
  const fingerprint = state.run.pending.fingerprint;
  state.runGate.stopWatcher();
  setConnection("busy", approved ? "记录批准" : "记录拒绝");
  try {
    const projection = await api(`/v1/runs/${encodeURIComponent(context.runId)}/approval`, {
      method: "POST",
      body: { fingerprint, approved },
...
async function resumeRun() {
  ...
  try {
    const projection = await api(`/v1/runs/${encodeURIComponent(context.runId)}/resume`, {
      method: "POST",
      body: {},
      signal: context.controller.signal,
    });
    ...
    await loadEvents(context);
    if (!contextIsActive(context)) return;
    if (!terminalStates.has(projection.state) && !projection.pause_reason) startWatching(context);
...
async function loadEvents(context = currentRunContext(), signal = context && context.controller.signal) {
  if (!contextIsActive(context) || !signal || signal.aborted ||
      !state.eventState || state.eventState.runId !== context.runId) return;
  const after = eventCursor();
  const payload = await api(`/v1/runs/${encodeURIComponent(context.runId)}/events?after_seq=${after}`, {
    signal,
  });
  if (!contextIsActive(context) || signal.aborted) return;
  if (!payload || payload.run_id !== context.runId || payload.after_seq !== after ||
      !Number.isSafeInteger(payload.latest_seq) || payload.latest_seq < 0 ||
      !Array.isArray(payload.events)) {
    throw new EventProjectionError("invalid_event_batch", "event history response is invalid");
  }
  payload.events.forEach(addEvent);
  if (state.run && state.run.run_id === context.runId) {
    state.run.latest_seq = Math.max(state.run.latest_seq || 0, payload.latest_seq, eventCursor());
  }
}

async function streamEvents(context, signal = context && context.controller.signal) {
  if (!contextIsActive(context) || !signal || signal.aborted) return;
  const response = await fetch(`/v1/runs/${encodeURIComponent(context.runId)}/events?after_seq=${eventCursor()}`, {
  ...
  while (true) {
    ...
    const framed = frameSseText(frameState, decoded, done);
    frameState = framed.state;
    for (const block of framed.blocks) {
...
async function watchRun(watcher) {
  const context = watcher.context;
  const signal = watcher.controller.signal;
  let retry = 600;
  while (watcherIsActive(watcher)) {
    try {
      const projection = await api(`/v1/runs/${encodeURIComponent(context.runId)}`, {
        signal,
      });
      if (!watcherIsActive(watcher)) break;
      setRunProjection(projection, context);
      await loadEvents(context, signal);
      if (!watcherIsActive(watcher)) break;
      if (state.run && (terminalStates.has(state.run.state) || state.run.pause_reason)) break;
      setConnection("live", "事件流已连接");
      await streamEvents(context, signal);
      if (!watcherIsActive(watcher)) break;
      retry = 600;
      const current = await api(`/v1/runs/${encodeURIComponent(context.runId)}`, {
        signal,
      });
      if (!watcherIsActive(watcher)) break;
      setRunProjection(current, context);
      if (state.run && (terminalStates.has(state.run.state) || state.run.pause_reason)) break;
    } catch (error) {
      if (!watcherIsActive(watcher)) break;
      if (error instanceof EventProjectionError) {
        showError(error, "事件投影校验失败");
        break;
      }
      if (error instanceof ApiError && error.code === "cursor_ahead") {
        showError(error, "耐久事件游标校验失败");
        break;
      ...
      try {
        ...
        if (!watcherIsActive(watcher) && delayError.name === "AbortError") break;
        throw delayError;
      }
      retry = Math.min(retry * 1.7, 5000);
    }
  }
  if (watcherIsActive(watcher) && state.run && state.run.run_id === context.runId && terminalStates.has(state.run.state)) {
    setConnection("live", "运行已落盘");
    await loadHistory();
  }
}

async function refreshCurrentRun(context = currentRunContext()) {
  if (!contextIsActive(context)) return;
  try {
    const projection = await api(`/v1/runs/${encodeURIComponent(context.runId)}`, {
      signal: context.controller.signal,
...
async function openRun(runId) {
  const context = activateRun(runId);
  renderTimeline();
