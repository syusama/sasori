...
const studioModal = $("#workflow-studio");
...
let studioModalActive = false;

function studioFocusableElements() {
  return $$([
    "a[href]",
    "button:not([disabled])",
    "input:not([disabled])",
  ...
  ].join(","), studioModal).filter((item) => !item.hidden && item.getClientRects().length);
}

function syncStudioModalState() {
  const open = !studioModal.hidden;
  if (open === studioModalActive) return;
  studioModalActive = open;
  if (open) {
    studioBackground.forEach((region) => {
      region.inert = true;
...
new MutationObserver(syncStudioModalState).observe(studioModal, {
  attributes: true,
  attributeFilter: ["hidden"],
});
document.addEventListener("keydown", (event) => {
  if (!studioModalActive || event.key !== "Tab") return;
  const focusable = studioFocusableElements();
  if (!focusable.length) {
    event.preventDefault();
    studioModal.focus();
    return;
  ...
  if (event.shiftKey && (document.activeElement === first || !studioModal.contains(document.activeElement))) {
    event.preventDefault();
    last.focus();
  } else if (!event.shiftKey && (document.activeElement === last || !studioModal.contains(document.activeElement))) {
    event.preventDefault();
    first.focus();
  }
}, true);
...
renderSurface = function renderSurfaceWithCapabilityCenter(app) {
  ...
  const mcpPlugins = (app.plugins || []).filter((plugin) =>
    plugin.capability_kind === "mcp_transport");
  ...
  if (!mcpPlugins.length) {
    ...
    mcpPlugins.forEach((plugin) => {
      ...
      card.append(
        ...
        element("p", "", `${plugin.id} · ${plugin.transport_type || "unspecified"} · ${plugin.execution_mode}`),
      );
      mcp.append(card);
    });
  }
